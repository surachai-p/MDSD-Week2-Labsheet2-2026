...
class _MainScreenState extends State<MainScreen> {
  ...
  final List<Widget> _pages = const [
    ...
    CounterPage(),
    FormPage(),
    AboutPage(), // [โจทย์ A] เพิ่มหน้า About เป็น Tab ที่ 4
  ];
...
class AboutPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('เกี่ยวกับผู้พัฒนา'),
        backgroundColor: Theme.of(context).colorScheme.inversePrimary,
      ),
      body: Center(
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              // Avatar ตัวอักษรแรกของชื่อ "N" (Nureen)
              CircleAvatar(
                radius: 50,
    ...
    return Card(
      child: Padding(
        ...
        child: Center(
          child: Column(
            children: [
              const Icon(Icons.access_time, size: 32, color: Colors.teal),
              Text(
        ...
        children: [
          Text(
            ...
            style: TextStyle(
              fontSize: 80,
              fontWeight: FontWeight.bold,
              color: _count >= 0 ? Colors.teal : Colors.red,
            ),
          ),
...
// ─── GreetingForm (StatefulWidget - โจทย์ C) ─────────────
...
class _GreetingFormState extends State<GreetingForm> {
  ...
  String _selectedLanguage = 'ไทย'; // ภาษาเริ่มต้น
  String _greeting = '';
  String _error = '';
  ...
  void _submit() {
    ...
    setState(() {
      if (name.isEmpty) {
        ...
        // คำนวณคำทักทายตามภาษาที่เลือก [โจทย์ C]
        if (_selectedLanguage == 'ไทย') {
          final period = h < 12
              ? 'ตอนเช้า'
              : h < 17
